...
export const personalizedSwimSuitCmd = new Command("personalized-swimsuit")
  .summary("Personalized swimsuit — transform a person photo into a bikini model image or video")
  .description(
    ...
    "  weshop personalized-swimsuit --image ./person.png --generated-type video --no-wait"
  )
  .requiredOption("--image <path|url>", "Input portrait or full-body photo — local file path or URL")
  .option("--prompt <text>", "Describe the desired swimsuit scene")
  ...
  .action(async (opts) => {
    ...
    await executeRun("personalized-swimsuit", "v1.0", { image: opts.image, wait: opts.wait }, params, extraInput);
  });
